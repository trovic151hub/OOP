...
function StatCard({ label, value, sub, icon: Icon, color, trend }) {
  ...
  const colors = {
    ...
    red:    { bg: 'bg-red-50 dark:bg-red-500/12',    icon: 'text-red-500',    border: 'border-red-100 dark:border-red-500/20' },
  }
  ...
  return (
    <div className="card p-3 sm:p-5 flex items-start justify-between min-w-0">
      <div className="min-w-0 flex-1 pr-2">
        ...
        {isNumber
          ? <p ref={ref} className="text-2xl sm:text-3xl font-extrabold text-slate-800 dark:text-slate-200 mb-0.5 sm:mb-1">0</p>
          : <p className="text-lg sm:text-2xl font-extrabold text-slate-800 dark:text-slate-200 mb-0.5 sm:mb-1 truncate">{value}</p>
        }
        {sub && <p className="text-[10px] sm:text-xs text-slate-400 dark:text-slate-600 truncate">{sub}</p>}
        {trend && (
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: 'In Progress',  value: isDoctor ? todayAppts.filter(a=>a.status==='In Progress').length : inProgress,  color: 'bg-blue-50 dark:bg-blue-500/12 text-blue-600',    icon: Clock },
          { label: 'Completed',    value: todayAppts.filter(a => a.status === 'Completed').length,          color: 'bg-emerald-50 dark:bg-emerald-500/12 text-emerald-600', icon: CheckCircle },
          ...
          <div key={label} className="card p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-extrabold text-slate-800 dark:text-slate-200">{value}</p>
            </div>
          </div>
          )
        })}
        ...
        <div className="bg-violet-50 dark:bg-violet-500/12 border border-violet-200 dark:border-violet-500/30 rounded-xl px-4 py-3 mb-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              {overdueFollowUps.length > 0 && <span className="text-[10px] bg-red-100 dark:bg-red-500/18 text-red-700 font-bold px-1.5 py-0.5 rounded-full">{overdueFollowUps.length} overdue</span>}
              {todayFollowUps.length > 0 && <span className="text-[10px] bg-violet-100 dark:bg-violet-500/18 text-violet-700 font-bold px-1.5 py-0.5 rounded-full">{todayFollowUps.length} today</span>}
              {upcomingFollowUps.length > 0 && <span className="text-[10px] bg-blue-100 dark:bg-blue-500/18 text-blue-700 font-bold px-1.5 py-0.5 rounded-full">{upcomingFollowUps.length} this week</span>}
            </div>
            <div className="flex flex-wrap gap-2">
              {[...overdueFollowUps, ...todayFollowUps, ...upcomingFollowUps].slice(0, 6).map(a => (
                <span key={a.id} className={`text-xs font-semibold px-2 py-0.5 rounded-lg border ${a.followUpDate < today ? 'bg-red-50 dark:bg-red-500/12 border-red-200 dark:border-red-500/30 text-red-700' : a.followUpDate === today ? 'bg-violet-100 dark:bg-violet-500/18 border-violet-300 text-violet-800' : 'bg-white dark:bg-slate-800 border-violet-200 dark:border-violet-500/30 text-violet-700'}`}>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              {lowStock.slice(0, 6).map(i => (
                <span key={i.id} className="text-xs bg-white dark:bg-slate-800 border border-amber-200 dark:border-amber-500/30 text-amber-700 font-semibold px-2 py-0.5 rounded-lg">
          ...
          <div className="card p-5 lg:col-span-2">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={last6Months} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} tickFormatter={v => fmt(v)} width={44} />
