...
import {
  ...
  Stethoscope, Home, Menu, X, Pencil, Plus, Send, CreditCard, Bell, Upload, NotebookPen,
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 overflow-hidden">
      ...
      <div className={`min-h-screen min-w-0 flex flex-col transition-[margin-left] duration-300 ${sidebarCollapsed ? 'md:ml-[72px]' : 'md:ml-60'}`}>
        <header className={`fixed top-0 left-0 right-0 z-40 h-16 bg-white/95 dark:bg-slate-800/95 backdrop-blur border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-4 sm:px-6 transition-all duration-300 ${sidebarCollapsed ? 'md:left-[72px]' : 'md:left-60'}`}>
        ...
        <main className="flex-1 pt-20 pb-20 md:pb-8 px-4 sm:px-6 overflow-y-auto">
          <div className="max-w-5xl mx-auto">

            {page === 'overview' && (
              <>
                <div className="mb-5 rounded-3xl bg-white/90 dark:bg-slate-800/95 border border-white dark:border-slate-700 shadow-sm overflow-hidden">
                  <div className="p-5 sm:p-6 flex flex-col lg:flex-row lg:items-center gap-5">
                    <div className="flex-1 min-w-0">
                      ...
                      <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-slate-100">Welcome back, {patientName.split(' ')[0]}</h1>
                ...
                {patientProfile && (
                  <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 sm:p-5 mb-5">
                ...
                {patientProfile && missingProfileFields.length > 0 && (
                  <div className="bg-amber-50 dark:bg-amber-500/12 border border-amber-200 dark:border-amber-500/30 rounded-2xl px-4 py-3 mb-5 flex items-start justify-between gap-3">
                ...
                {patientTimeline.length > 0 && (
                  <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 sm:p-5 mb-6">
            ...
            {page === 'appointments' && (
              <>
                ...
                {myAppts.length === 0 ? (
                  <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center py-20 px-5 text-center text-slate-400 dark:text-slate-600">
                  ...
                  <div className="flex flex-col gap-3">
                    {myAppts.sort((a,b) => b.date.localeCompare(a.date)).map(a => {
                      ...
                      return (
                      <div key={a.id} className={`bg-white dark:bg-slate-800 rounded-2xl border p-4 sm:p-5 ${waitingReview ? 'border-amber-200 dark:border-amber-500/30' : 'border-slate-200 dark:border-slate-700'}`}>
                        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                        <div className="w-full sm:w-28 rounded-2xl bg-teal-50 dark:bg-teal-500/12 border border-teal-100 dark:border-teal-500/30 px-4 py-3 text-teal-700 dark:text-teal-400 flex-shrink-0">
                        ...
                        <div className="flex flex-col items-end gap-2 flex-shrink-0">
                          {a.status === 'Scheduled' && (
                            <div className="flex flex-col gap-1.5">
                              ...
                              <button onClick={() => openAppointmentChange(a, 'cancel')} className="text-xs font-semibold text-red-500 hover:text-red-600 px-3 py-1.5">
                                Cancel
                              </button>
                            </div>
                          )}
              ...
              <>
                ...
                {myBills.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-5">
                    <div className={`rounded-3xl border p-5 sm:p-6 ${pendingTotal > 0 ? 'bg-amber-50 dark:bg-amber-500/12 border-amber-200 dark:border-amber-500/30' : 'bg-emerald-50 dark:bg-emerald-500/12 border-emerald-200 dark:border-emerald-500/30'}`}>
                ...
                {myDocs.length === 0 ? (
                  ...
                  <>
                    ...
                    {myDocs.sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(d => {
                      ...
                      return (
                      <div key={d.id} className={`bg-white dark:bg-slate-800 rounded-2xl border p-4 sm:p-5 flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4 ${rejected ? 'border-red-200 dark:border-red-500/30' : 'border-slate-200 dark:border-slate-700'}`}>
        ...
        {selectedInvoice && (
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-3">
              <button disabled={payingInvoice} onClick={() => setPaymentModal(false)} className="btn-ghost flex-1 justify-center disabled:opacity-60 disabled:cursor-not-allowed">Cancel</button>
      ...
      <Modal open={documentModal} onClose={() => !uploadingDocument && setDocumentModal(false)} title="Upload Document" icon={Upload} accentColor="teal">
        ...
        <div className="flex gap-3 mt-5">
          <button disabled={uploadingDocument} onClick={() => setDocumentModal(false)} className="btn-ghost flex-1 justify-center disabled:opacity-60 disabled:cursor-not-allowed">Cancel</button>
      ...
      >
        {selectedAppointment && (
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-3">
              <button disabled={sendingAppointmentChange} onClick={() => setAppointmentChangeModal(false)} className="btn-ghost flex-1 justify-center disabled:opacity-60 disabled:cursor-not-allowed">Cancel</button>
      ...
      <Modal open={profileModal} onClose={() => setProfileModal(false)} title="Update Profile" icon={User} accentColor="teal">
        ...
        <div className="flex gap-3 mt-5">
          <button disabled={savingProfile} onClick={() => setProfileModal(false)} className="btn-ghost flex-1 justify-center disabled:opacity-60 disabled:cursor-not-allowed">Cancel</button>
          <button onClick={saveProfile} disabled={savingProfile || !patientProfile} className="btn-primary flex-1 justify-center disabled:opacity-60 disabled:cursor-not-allowed">
            {savingProfile ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : 'Save Profile'}
          </button>
        </div>
      </Modal>
      ...
      <Modal open={requestModal} onClose={() => setRequestModal(false)} title="Request Appointment" icon={Calendar} accentColor="teal">
        ...
        <div className="flex gap-3 mt-5">
          <button disabled={sendingRequest} onClick={() => setRequestModal(false)} className="btn-ghost flex-1 justify-center disabled:opacity-60 disabled:cursor-not-allowed">Cancel</button>
...
function QuickAction({ icon: Icon, title, subtitle, tone = 'teal', onClick, action }) {
  ...
  return (
    <button
      onClick={onClick}
      className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 text-left hover:border-teal-200 dark:hover:border-teal-500/40 hover:shadow-sm transition-all"
    >
      <div className={`w-10 h-10 rounded-xl border flex items-center justify-center mb-3 ${colors}`}>
        <Icon size={17} />
      </div>
      ...
      <p className="text-xs font-bold text-teal-600 mt-3 flex items-center gap-1">
...
function BillCard({ invoice, payable, settings, onPay }) {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-2xl border p-4 sm:p-5 ${overdue ? 'border-red-200 dark:border-red-500/30' : payable ? 'border-amber-200 dark:border-amber-500/30' : 'border-slate-200 dark:border-slate-700'}`}>
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        <div className={`w-full sm:w-36 rounded-2xl border px-4 py-3 flex-shrink-0 ${paid ? 'bg-emerald-50 dark:bg-emerald-500/12 border-emerald-100 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400' : overdue ? 'bg-red-50 dark:bg-red-500/12 border-red-100 dark:border-red-500/30 text-red-700 dark:text-red-400' : 'bg-amber-50 dark:bg-amber-500/12 border-amber-100 dark:border-amber-500/30 text-amber-700 dark:text-amber-400'}`}>
...
function PrescriptionCard({ prescription }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 sm:p-5">
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        <div className={`w-full sm:w-32 rounded-2xl border px-4 py-3 flex-shrink-0 ${prescription.status === 'Active' ? 'bg-teal-50 dark:bg-teal-500/12 border-teal-100 dark:border-teal-500/30 text-teal-700 dark:text-teal-400' : 'bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-500'}`}>
...
function LabResultCard({ lab }) {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-2xl border p-4 sm:p-5 ${abnormal ? 'border-red-200 dark:border-red-500/30' : 'border-slate-200 dark:border-slate-700'}`}>
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        <div className={`w-full sm:w-32 rounded-2xl border px-4 py-3 flex-shrink-0 ${abnormal ? 'bg-red-50 dark:bg-red-500/12 border-red-100 dark:border-red-500/30 text-red-700 dark:text-red-400' : 'bg-violet-50 dark:bg-violet-500/12 border-violet-100 dark:border-violet-500/30 text-violet-700 dark:text-violet-400'}`}>
...
function NotificationCard({ notification, onOpen }) {
  ...
  return (
    <button
      ...
      className={`bg-white dark:bg-slate-800 rounded-2xl border p-4 sm:p-5 flex items-start gap-3 sm:gap-4 text-left hover:border-teal-200 dark:hover:border-teal-500/40 transition-colors ${unread ? 'border-teal-200 dark:border-teal-500/30' : 'border-slate-200 dark:border-slate-700'}`}
    >
