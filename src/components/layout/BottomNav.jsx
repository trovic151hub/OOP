...
export default function BottomNav({ currentPage, onNavigate, onMenuOpen }) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex items-center md:hidden safe-bottom">
        ...
        return (
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-2.5 transition-colors min-h-[56px] touch-manipulation
              ${active ? 'text-teal-600' : 'text-slate-400 dark:text-slate-600 hover:text-slate-600 dark:hover:text-slate-400'}`}>
      ...
      <button
        onClick={onMenuOpen}
        className="flex-1 flex flex-col items-center justify-center gap-0.5 py-2.5 text-slate-400 dark:text-slate-600 hover:text-slate-600 dark:hover:text-slate-400 min-h-[56px] touch-manipulation">
