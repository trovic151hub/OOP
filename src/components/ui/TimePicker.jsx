...
export default function TimePicker({ value, onChange, min, max, placeholder = 'Select time' }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={value ? 'text-slate-700' : 'text-slate-400'}>{value ? formatTimeLabel(value) : placeholder}</span>
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-[90]" onClick={() => setOpen(false)} />
          <div className="absolute top-full mt-1.5 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-lg z-[100] overflow-y-auto max-h-56 py-1">
            {slots.map(s => (
              <button
                type="button"
                key={s}
                onClick={() => { onChange(s); setOpen(false) }}
                className={`w-full text-left px-3.5 py-2 text-sm transition-colors ${value === s ? 'bg-teal-50 text-teal-700 font-semibold' : 'text-slate-600 hover:bg-slate-50'}`}
              >
                {formatTimeLabel(s)}
              </button>
            ))}
