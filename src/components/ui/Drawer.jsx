...
export default function Drawer({ open, onClose, title, subtitle, children, width = 'max-w-3xl', footer }) {
  ...
  return (
    <>
      {open && (
        <div
          className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[58] transition-opacity"
          onClick={onClose}
        />
      )}
      <div
        className={`fixed top-0 right-0 h-full ${width} w-full bg-white dark:bg-slate-800 shadow-2xl z-[59] flex flex-col transform transition-transform duration-300 ease-in-out
    ...
    <div className="flex gap-0 border-b border-slate-200 dark:border-slate-700 px-6 bg-white dark:bg-slate-800 flex-shrink-0">
      {tabs.map(tab => (
        ...
        >
          {tab.label}
          {tab.count !== undefined && (
            <span className={`ml-1.5 text-xs font-bold px-1.5 py-0.5 rounded-full ${active === tab.id ? 'bg-teal-100 dark:bg-teal-500/18 text-teal-700' : 'bg-slate-100 dark:bg-slate-900 text-slate-500'}`}>
              {tab.count}
            </span>
          )}
        </button>
