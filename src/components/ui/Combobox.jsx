    ...
    <div className="relative">
      ...
      {open && filtered.length > 0 && (
        <>
          ...
          <div className="absolute top-full mt-1.5 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-lg z-[100] overflow-hidden max-h-52 overflow-y-auto py-1">
            {filtered.slice(0, 8).map((o, i) => (
              <button
                type="button"
                key={o.id || i}
                onClick={() => { onChange(getLabel(o)); setOpen(false) }}
                className="w-full text-left px-3.5 py-2 text-sm hover:bg-slate-50 transition-colors flex items-center justify-between gap-3"
              >
                <span className="font-medium text-slate-700">{getLabel(o)}</span>
                {getSub && <span className="text-xs text-slate-400 flex-shrink-0">{getSub(o)}</span>}
              </button>
            ))}
          </div>
        </>
